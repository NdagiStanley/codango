...
class TestPairProgram(TestCase):
    def setUp(self):
        ...
        self.session = Session.objects.create(initiator=self.user, session_name="A very Random session")
    ...
    def test_can_read_a_session(self):
        pair_session = Session.objects.get(session_name="A very Random session")
        self.assertIsInstance(pair_session, Session)
